  ...
  parse(lines: NormalizedLine[]): FailureEvent[] {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      for (let cursor = index + 1; cursor < Math.min(lines.length, index + 28); cursor += 1) {
        if (cursor > index + 1 && (TEST_NAME.test(lines[cursor].normalized) || FAIL_FILE.test(lines[cursor].normalized))) break;
        if (/^\s*(Tests:|Test Suites:)/.test(lines[cursor].normalized)) break;
      ...
      const stack = block.map((entry) => entry.normalized.match(STACK)).find(Boolean);
